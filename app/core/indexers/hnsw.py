...
from app.config.indexer_hparams import HNSWIndexerHParams, get_default_hparams
from app.core.indexers.indexer import DistanceMetric, Indexer
from app.core.models.chunk import Chunk

hparams: HNSWIndexerHParams = get_default_hparams(IndexerType.HNSW)


class HNSWIndexer(Indexer):
    """
    ...
    def __init__(
        ...
        # HNSW parameters
        self._m: int = m  # Maximum number of connections per node
        self._m_max0: int = m * 2  # Max connections for ground layer (typically 2*m)
        self._ef_construction: int = (
            ef_construction  # Size of dynamic candidate list during construction
        )
        self._max_level: int = max_level  # Maximum layer in the graph
        self._level_mult: float = level_mult  # Level multiplier

        # Graph structure: Dict of dicts for each level
        # {level: {node_id: set(neighbor_ids)}}
        self._graph: dict[int, dict[str, set[str]]] = {
    ...
    def add(self, chunk_id: str, embedding: np.ndarray, _skip_lock: bool = False):
        ...
        Args:
            ...
            _skip_lock: Internal parameter. Set to True when called
        ...
        if chunk_id in self.embeddings:
            warnings.warn(
                f"Chunk {chunk_id} already exists in the index. Skipping.", UserWarning
            )
            return
        ...
        if len(self.embeddings) == 1:
            ...
            for lvl in range(level + 1):
                self._graph[lvl][chunk_id] = set()
            return

        # Find nearest neighbors to insert the new element in the graph
        curr_node = self._entry_point
        curr_dist = self._calculate_distance_raw(
            normalized_embedding, self.embeddings[curr_node], DistanceMetric.EUCLIDEAN
        )

        # Traverse from top to level l+1, searching for closest node
        for lvl in range(self._max_layer, level, -1):
            changed = True
            while changed:
                changed = False

                # Check neighbors at this level
                neighbors = self._graph[lvl][curr_node]
                for neighbor in neighbors:
                    dist = self._calculate_distance_raw(
                        normalized_embedding,
                        self.embeddings[neighbor],
        ...
        for lvl in range(min(level, self._max_layer), -1, -1):
            # Find ef_construction nearest neighbors at level lc
            neighbors = self._search_level(
                normalized_embedding, curr_node, self._ef_construction, lvl
            )

            # Initialize connections for the new node at this level
            if chunk_id not in self._graph[lvl]:
                self._graph[lvl][chunk_id] = set()

            # Select M nearest neighbors
            neighbors_to_connect = self._select_neighbors(
                neighbors, self._m if lvl > 0 else self._m_max0
            )

            # Add bidirectional connections
            for neighbor_id, _ in neighbors_to_connect:
                self._graph[lvl][chunk_id].add(neighbor_id)

                # Initialize if neighbor doesn't have connections at this level
                if neighbor_id not in self._graph[lvl]:
                    self._graph[lvl][neighbor_id] = set()

                self._graph[lvl][neighbor_id].add(chunk_id)

                # Ensure nodes don't exceed max connections
                max_m = self._m if lvl > 0 else self._m_max0
                if len(self._graph[lvl][neighbor_id]) > max_m:
                    # Prune connections to keep only m/m_max0 closest
                    self._graph[lvl][neighbor_id] = set(
                        [
                            node_id
                            for node_id, _ in self._select_neighbors(
                                [
                                    (
                                        nid,
                                        self._calculate_distance_raw(
                                            self.embeddings[neighbor_id],
                                            self.embeddings[nid],
                                            DistanceMetric.EUCLIDEAN,
                                        ),
                                    )
                                    for nid in self._graph[lvl][neighbor_id]
                                ],
                                max_m,
                            )
                        ]
    ...
    def delete(self, chunk_id: str, _skip_lock: bool = False):
        ...
        for lvl in range(level + 1):
            if lvl in self._graph and chunk_id in self._graph[lvl]:
                # Get all neighbors at this level
                neighbors = self._graph[lvl][chunk_id]

                # Remove bidirectional connections
                for neighbor in neighbors:
                    if neighbor in self._graph[lvl]:
                        self._graph[lvl][neighbor].discard(chunk_id)

                # Delete node from this level
                del self._graph[lvl][chunk_id]
    ...
    def search(
        ...
        with self._lock:
            ...
            curr_node = self._entry_point
            curr_dist = self._calculate_distance_raw(
                query_normalized, self.embeddings[curr_node], distance_metric
            )
            ...
            for level in range(self._max_layer, 0, -1):
                ...
                while changed:
                    ...
                    # Check neighbors at this level
                    neighbors = (
                        self._graph[level][curr_node]
                        if curr_node in self._graph[level]
                        else set()
                    )
                    for neighbor in neighbors:
                        dist = self._calculate_distance_raw(
                            query_normalized, self.embeddings[neighbor], distance_metric
                        )
                        if dist < curr_dist:
                            curr_dist = dist
                            curr_node = neighbor
            ...
            nearest = self._search_level(
                query_normalized, curr_node, ef_search, 0, distance_metric
            )
            ...
            if len(nearest) == 0:
                ...
                warnings.warn(
                    f"""
                    Only {len(nearest)} nearest neighbors found for query embedding.
                    Returning {len(nearest)} nearest neighbors.
                    """,
                    UserWarning,
                )
    ...
    def _search_level(
        ...
        distance_to_entry = self._calculate_distance_raw(
            query_vector, self.embeddings[entry_point], distance_metric
        )
            ...
            if curr_node in self._graph[level]:
                for neighbor in self._graph[level][curr_node]:
                    if neighbor not in visited:
                        ...
                        dist = self._calculate_distance_raw(
                            query_vector, self.embeddings[neighbor], distance_metric
                        )

                        # If we haven't found ef neighbors yet or if the
                        # neighbor is closer than our furthest neighbor
    ...
    def _calculate_distance_raw(
        self, vec1: np.ndarray, vec2: np.ndarray, distance_metric: DistanceMetric
    ) -> float:
