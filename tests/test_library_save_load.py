...
def test_cross_library_search(create_test_library):
    ...
    for metric in metrics:
        # Search in HSNW library with different metrics
        results = hsnw_library.indexer.search(query_embedding, k=3, distance_metric=metric)
...
# Parameterized test to check both indexers in a single test
@pytest.mark.parametrize("indexer_type", ["flat", "hsnw"])
def test_indexer_persistence(create_test_library, temp_dir, indexer_type):
    """Test that both types of indexers persist correctly"""
    ...
    indexer_class = HSNWIndexer if indexer_type == "hsnw" else FlatIndexer
    loaded_library = Library(
        name="Empty_Library",
        documents={},
        chunks={},
