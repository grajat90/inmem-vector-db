...
def mock_embeddings(test_chunks_df):
    ...
    for _, row in test_chunks_df.iterrows():
        ...
        embedding = np.array(
            [float(text_hash / 10000)] * 32, dtype=np.float32
        )  # 32-dim mock embedding
...
def create_test_library(test_chunks_df, mock_embeddings):
    def _create_library(name, indexer_type="flat"):
        ...
        if indexer_type == "hnsw":
            indexer = HNSWIndexer(name="hnsw_test_indexer")
        ...
        for document_name, group in document_groups:
            ...
            document = Document(
                ...
                chunks=[],
                metadata=DocumentMetadata(source="test", tags=["test"]),
            )
            ...
            for _, row in group.iterrows():
                chunk = Chunk(
                    ...
                    metadata=ChunkMetadata(source="test", tags=["test"]),
                )
...
def test_save_load_hnsw_indexer(create_test_library, temp_dir):
    """Test saving and loading a library with an HNSW indexer"""
    # Create a test library with an HNSW indexer
    original_library = create_test_library("Test_HNSW_Save_Load", "hnsw")

    # Define a save path
    save_path = os.path.join(temp_dir, "test_hnsw_library.pkl")
    ...
    # (we don't test exact matches due to HNSW's approximate nature)
    random_chunk_id = list(loaded_library.chunks.keys())[0]
    query_embedding = loaded_library.chunks[random_chunk_id].embedding
...
def test_save_load_lsh_indexer(create_test_library, temp_dir):
    ...
    assert len(loaded_library.indexer._hash_tables) == len(
        original_library.indexer._hash_tables
    )
...
def test_cross_library_search(create_test_library):
    ...
    # Create a library with an HNSW indexer
    hnsw_library = create_test_library("Test_HNSW_Cross_Search", "hnsw")
    ...
    hnsw_results = hnsw_library.search(query_embedding, k=5)
...
@pytest.mark.parametrize("indexer_type", ["flat", "hnsw", "lsh"])
def test_indexer_persistence(create_test_library, temp_dir, indexer_type):
    ...
    assert type(loaded_library.indexer) is type(library.indexer)
