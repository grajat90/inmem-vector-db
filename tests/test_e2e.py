...
def create_library(client: TestClient) -> Response:
    return create_library_with_indexer(client, "hnsw")


def test_create_library():
    """Test creating a library with HNSW indexer (legacy test)"""
    global create_library_response
    create_library_response = create_library(client)
    assert create_library_response.status_code == status.HTTP_200_OK
    assert create_library_response.json()["name"] == "Test Library - HNSW"
    # Store the library ID for later tests
    library_ids["hnsw"] = create_library_response.json()["library_id"]
...
def test_update_library():
    ...
    update_request = {
        ...
        "metadata": {"tags": ["test", "flat", "updated"]},
        "indexer": "flat",
    }
...
def test_search_library():
    ...
    expected_chunk_text = (
        ...
        + " consumes significant amounts of electricity and"
        + " may be contributing to global warming."
        + " Some cryptocurrencies are, by design, not reliant"
        + " on this type of processing power."
    )
...
def test_search_with_lsh_indexer():
    ...
    # Check for the specific chunk text in the results
    # (may be different than HNSW due to approximate nature)
...
def test_search_with_different_metrics():
    ...
    # Test with HNSW indexer (legacy)
    library_id = library_ids["hnsw"]
...
def test_add_document_to_library():
    ...
    document_request = {
        ...
        "metadata": {"source": "test", "tags": ["new", "added"]},
    }
...
def test_add_chunk_to_document():
    ...
    chunk_request = {
        ...
        "metadata": {"source": "test", "tags": ["new", "added", "chunk"]},
    }
    ...
    assert get_chunk_response.status_code == status.HTTP_200_OK
    ...
    chunk_request = {
        "text": "This is a new test chunk added to the "
        + "LSH indexed library to test functionality.",
...
def test_update_document():
    ...
    update_request = {
        ...
        "metadata": {"source": "test_update", "tags": ["test", "updated"]},
    }

    response = client.put(
        f"/libraries/{library_id}/documents/{document_id}", json=update_request
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_invalid_library_id():
    ...
    assert invalid_id in response.json()["error"]["details"]["library_id"]
...
def test_invalid_document_id():
    ...
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert invalid_id in response.json()["error"]["details"]["document_id"]
...
def test_invalid_chunk_id():
    ...
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert invalid_id in response.json()["error"]["details"]["chunk_id"]
...
def test_invalid_search_metric():
    ...
    assert "Invalid distance metric" in response.text
...
# The reindex_locks is expected to be a Dict[str, object]
# but is actually a Dict[str, asyncio.Lock]
...
def test_search_performance():
    ...
    for indexer_type in ["hnsw", "flat", "lsh"]:
